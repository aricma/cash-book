        ...
        <div className="flex items-center space-x-2">
            {props.accounts.map((accountProps, index) => (
                <React.Fragment key={index}>
                    <div
                        className="min-w-[12rem] bg-blue-200 dark:bg-blue-900 p-4 rounded-md space-y-2 text-blue-600 dark:text-blue-200">
...
export const BookEntryDayView: React.FC<BookEntryDayViewProps> = (props) => {
    const cashInformation: Array<[title: string, value: string, icon?: IconType]> = [
        ...
        ['Diff', props.cashInfo.diff, (toNumber(props.cashInfo.diff) || 0) >= 0 ? IconType.TRENDING_UP_FILL : IconType.TRENDING_DOWN_FILL],
    ];
    return (
        <Disclosure>
            {({open}) => (
                <>
                    <div className="w-full block button button-xs flex items-center space-x-2">
                        <Disclosure.Button className="flex-grow flex items-center justify-between space-x-2">
                            ...
                            <Icon type={open ? IconType.CHEVRON_DOWN_FILL : IconType.CHEVRON_LEFT_FILL}
                                  className="w-5 h-5"/>
                        </Disclosure.Button>
                                    ...
                                    <React.Fragment key={index}>
                                        <div className="bg-blue-900 rounded-md text-blue-300 py-1 px-4">
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    {props[2] ? props[2] === IconType.TRENDING_UP_FILL ? (
                                                        <Icon type={IconType.TRENDING_UP_FILL}
                                                              className="w-5 h-5 text-green-600"/>
                                                    ) : (
                                                        <Icon type={IconType.TRENDING_DOWN_FILL}
                        ...
                        <div className="w-full px-2 text-2 grid grid-cols-[repeat(5,_minmax(min-content,_1fr))] gap-2">
                            {props.entries.map((bookingProps, index) => {
                                return (
                                    <React.Fragment key={index}>
                                        {index !== 0 &&
                                            <div className="col-span-5 border-b border-gray-300 dark:border-gray-600"/>}
