...
export const toBookingsViewProps = (appState: ApplicationState): BookEntriesViewProps => ({
    ...
    entries: Object.values(Object
        .values(appState.bookEntries.entries)
        .sort((a, b) => {
...
const toBookEntryViewProps = (appState: ApplicationState, date: string, transactions: { [transactionId: string]: number }) => {
    return Object.entries(transactions).map(([transactionId, value]): DataBookEntryViewProps | ErrorBookEnrtyViewProps => {
