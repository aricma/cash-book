...
export const toTransactionsViewProps = (
    ...
    templates: Object.values(appState.transactions.templates).map((template) =>
        toTemplateViewProps(appState, template, showCreateTransactionView),
    ),
});

const toTemplateViewProps = (
    appState: ApplicationState,
    template: Template,
    openModal: () => void,
): TemplateViewProps => ({
    ...
    transactions: compact(Object.values(template.transactions).map((transactionId) => {
...
const toTransactionViewProps = (
    appState: ApplicationState,
    template: Template,
    transaction: Transaction,
): TransactionViewProps | undefined => {
    const templateId = template.id;
    const [fromAccountId, toAccountId] = transaction.type === TransactionType.IN ? [transaction.accountId, template.cashierAccountId] : [template.cashierAccountId, transaction.accountId];
...
const validateCreateTemplate = (appState: ApplicationState): CreateTemplateValidationMap | undefined => {
...
const toCreateTransactionViewProps = (
    appState: ApplicationState, order: number, transaction: CreateTransaction,
): CreateTransactionViewProps => {
...
const validateCreateTransaction = (transaction: CreateTransaction): CreateTransactionValidationMap | undefined => {
