...
export const reducer: Reducer<TransactionsState, TransactionsAction> = (state, action) => {
	switch (action.type) {
			...
			const data = template.transactions.reduce(
				(data: CreateConfig, transactionId) => {
					...
					const cashierAccountId =
						transaction.type === TransactionType.IN ? transaction.toAccountId : transaction.fromAccountId;
					const otherAccountId =
						transaction.type === TransactionType.IN ? transaction.fromAccountId : transaction.toAccountId;
					return {
						...data,
		...
		case ApplicationActionType.TRANSACTIONS_CREATE_ORDER_DEC: {
			...
			if (transaction === undefined) return state;
			const prevTransaction =
				Object.values(state.create.transactions).find(({ order }) => order === transaction.order - 1) || undefined;
			if (prevTransaction === undefined) return state;
			return {
				...state,
				create: {
		...
		case ApplicationActionType.TRANSACTIONS_CREATE_TRANSACTION_ADD: {
			...
			const getLastOrderNumber = allTransactions.map(({ order }) => order).sort((a, b) => a - b)[lastIndex] || 0;
			return {
				...state,
				create: {
					...state.create,
		...
		case ApplicationActionType.TRANSACTIONS_CREATE_TEMPLATE_SUBMIT: {
			...
			const newTransactions = compactObject(
				Object.fromEntries(
					Object.values(state.create.transactions).map(
						(config): [transactionId: string, transaction: Transaction | undefined] => {
							...
							const fromAccountId =
								config.type === TransactionType.OUT ? state.create.cashierAccountId : config.accountId;
