...
import {CreateBookEntryViewProps, OverrideDateConfirmationModalViewProps} from './props';
import {ApplicationState, dispatch} from '../../applicationState';
import {DateWithoutTime} from '../../models/domain/date';
import {compact, toNumber, subtractDays} from '../../models/utils';


export const toCreateBookEntryViewProps = (appState: ApplicationState, showValidation: boolean, setShowValidation: () => void, openDateOverrideConfirmationModal: () => void): CreateBookEntryViewProps => {
    const validationMap = validateCreateBookEntry(appState);
    const needsDateOverrideConfirmation = validateIfDateExists(appState) !== undefined;
    const dateDisplay = appState.bookEntries.create.date === undefined ? undefined : DateWithoutTime.fromString(appState.bookEntries.create.date).toLocaleDateString('de-DE', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
    });
    const dateValue = appState.bookEntries.create.date === undefined ? "" : appState.bookEntries.create.date;
    return ({
        ...
        inputs: Object.values(appState.transactions.transactions).sort((a, b) => {
            ...
            return {
                ...
                placeholder: '',
                validation: showValidation ? validationMap?.transactions[transaction.id] : undefined,
                onChange: (value) => {
                    if (/[\d.]*/.test(value)) {
                        dispatch({
        ...
        submit: validationMap === undefined ? needsDateOverrideConfirmation ? {
            type: 'BUTTON_PROPS_TYPE',
            title: 'Submit',
            onSelect: () => {
...
export const toOverrideDateConfirmationModalViewProps = (appState: ApplicationState, closeModal: () => void): OverrideDateConfirmationModalViewProps => {
    const dateDisplay = appState.bookEntries.create.date === undefined ? "Found no date!" : DateWithoutTime.fromString(appState.bookEntries.create.date).toLocaleDateString('de-DE', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
...
const validateIfDateExists = (appState: ApplicationState): string | undefined => {
    ...
    return Object.keys(appState.bookEntries.entries).includes(appState.bookEntries.create.date) ? "Date already exists!" : undefined;
}

const validateDate = (appState: ApplicationState): string | undefined => {
    return appState.bookEntries.create.date === undefined ? "Date is missing!" : undefined;
}

const validateTransactions = (appState: ApplicationState): { [transactionId: string]: string | undefined } => {
    const requiredTransactionIds = Object.keys(appState.transactions.transactions);
...
const validateTransactionValue = (appState: ApplicationState): string | undefined => {
    ...
    const value = requiredTransactionIds.reduce((value, transactionId) => {
        ...
        const transactionValue = toNumber(appState.bookEntries.create.transactions[transactionId]) || 0;
