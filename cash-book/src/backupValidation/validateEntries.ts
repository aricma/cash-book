...
export const validateEntries: Validation = (state) => {
	...
	const reduceEntries =
		(fn: (entry: any) => Array<string>) =>
		(state: any): Array<string> =>
			Object.values(state).reduce((all: Array<string>, entries: any) => {
				return [
					...all,
					...Object.values(entries).reduce((all: Array<string>, entry: any) => [...all, ...fn(entry)], []),
				];
			}, []);
	const hasTransactionsForIds = reduceEntries((entry: any) => Object.keys(entry.transactions))(
		state.bookEntries
	...
	const hasFalseTransactionValueFormat = !checkValueFormat(
		reduceEntries((entry) => Object.values(entry.transactions))(state.bookEntries)
	);
	const hasFalseCashValueFormat = !checkValueFormat(
		reduceEntries((entry) => [entry.cash.start, entry.cash.end])(state.bookEntries)
	);
	if (hasTemplatesForIds && hasTransactionsForIds && !hasFalseTransactionValueFormat && !hasFalseCashValueFormat)
		return null;
	return {
		accounts: null,
